...
exports.getTeacher = (req, res) => {
  ...
  return res.status(200).json({
    ...
    hasDefaultPassword: teacher.hasDefaultPassword
  });
};

exports.updateDefaultPassword = async (req, res) => {
  const teacher = res.locals.teacher;
  if (teacher.hasDefaultPassword) {
    const { password } = req.body;
    if (!password || password.length < 8) {
      return handler(
        res,
        401,
        'Please provide password of at least 8 characters'
      );
    }
    ...
    try {
      ...
      teacher.hasDefaultPassword = false;
      teacher.lastPasswordChange = new Date();
      await teacher.save();
      const token = await jwt.sign(
        { email: teacher.email, issueDate: new Date() },
        '4RanDom$',
        { expiresIn: '1w' }
      );
    ...
    return handler(res, 401, 'Default Password has been changed previously');
  }
};
